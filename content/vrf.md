...
Random numbers are ***not*** natively allowed on-chain. This is because Solana is deterministic, every validator that runs your code needs to have the same result. So we have to look outside of the blockchain for our randomness. Why would we want randomness? Random numbers have a lot of use cases in blockchain development that may not be immediately apparent. Random numbers can be used in an NFT mint to prevent malicious users from predicting the mint order ahead of time or even to influence the result of on-chain games. So how do we get randomness? This is where Verifiable Random Functions (VRFs) come in!

VRFs offer developers a secure means of integrating diverse functionalities on-chain in a decentralized fashion. By leveraging VRFs, developers can create fascinating applications and enhance user experiences, enabled by the capability to introduce unpredictable random numbers in a trustworthy manner.
