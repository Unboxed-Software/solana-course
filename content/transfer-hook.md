...
- Transfer hook extension allows developers to implement custom logic whenever the token is transfered.
- The `TransferHook` interface is used to create transfer hook programs.
- Invoking a transfer instruction with a transfer hook requires passing extra accounts, such as `extra-account-metas`.

# Overview

The Token Extension program provides many new features to mint and token accounts. These features allow developers to add more functionality to their tokens. 

This lesson will focus on the `transfer-hook` extension. The `transfer-hook` extension allows custom logic on each transfer, opening the door for many use cases. You could use transfer hooks to mint, burn, or transfer tokens on each transfer or fail if a specific condition is not met.

This lesson will cover how to implement transfer hooks onchain and interact with them offchain. 

## Implementing transfer hooks onchain

The Token Extension program requires any trasnfer hook program to implement the `TransferHook` interface.

The Transfer Hook Interface provides a way for developers to implement custom instruction logic that is executed on every token transfer on a specific Mint Account. Since a transfer hook may affect other accounts not directly associated with the transfer, the interface requires a list of extra accounts be passed into the hook. 

This interface has the following [instructions](https://github.com/solana-labs/solana-program-library/blob/master/token/transfer-hook/interface/src/instruction.rs):
...
While it's not required to implement the `InitializeExtraAccountMetaList` instruction, it is required to have the `extra_account_meta_list`. This list can be created by any instruction on a Transfer Hook program. However, the Program Derived Address (PDA) for the account must be derived using the following seeds:
...
Storing the extra accounts in the predefined PDA allows the client to add the accounts to the token transfer instruction (more on this in the offchain section).

In this section, we will cover how to use Anchor to implement the `initialize_extra_account_meta_list`. We will then take the list and create the execute instruction (that Anchor renames to `transfer_hook`). Finally, we'll create a `fallback` instruction to handle Cross-Program Invocation (CPI). 
...
1. `ExtraAccountMeta::new_with_pubkey`: Directly store the account address.
2. `ExtraAccountMeta::new_with_seeds`: Store the seeds to derive a PDA of the program itself.
3. `ExtraAccountMeta::new_external_pda_with_seeds`: Store the seeds to derive a PDA for a program other than the Transfer Hook program.

Note that if you are going to pass the transfer's Mint account into the extra account list, the mint will still be deescalabted.

Now let's talk about the instruction itself and see some code to help us wrap our heads around it better.

In Anchor, each instruction may contain its own struct of required accounts. Anchor will help us parse and validate these accounts. Let's look at an example implementation of the `InitializeExtraAccountMetaList` struct. The struct will have the following accounts:
...
In this example, we'll see the accounts stored in three diferent ways: by directly storing the account address, storing the seeds to derive a PDA of the program itself, or storing the seeds to derive a PDA for a program other than the Transfer Hook program. If we want to store the seeds, we'd either store them as a PDA of the transfer hook program or a PDA of another program like an associated token account. We can do both by using `ExtraAccountMeta::new_with_seeds` and `ExtraAccountMeta::new_external_pda_with_seeds`, respectively, and pass the seeds to them:
...
Providing the seeds themselves can be challenging, so we will break it down to make it easier to understand. 

When storing seeds, the code calls both `new_with_seeds` and `new_external_pda_with_seeds`. Both methods require a list of seeds, a `is_signer` boolean, and `is_writable` boolean. `new_external_pda_with_seeds` requires an extra parameter `program_index` for the PDAs that are not of our program. To learn how we could pass the seeds, let's take a look at the source code itself:
...
4. **AccountData**: An argument to be resolved from the inner data of some account. This can be done by giving the index of the account, the start index of the data, and the length of the data we want to have as a seed.

You can take a look at the seed enum implementation from [spl_tlv_account_resolution::seeds::Seed](https://github.com/solana-labs/solana-program-library/blob/master/libraries/tlv-account-resolution/src/seeds.rs).

For the AccountKey and the AccountData enums, you need to also provide the account index. Here is the accounts vector again but with the indexes:
...
The main instruction for the transfer hook program in Anchor is `transfer_hook` (aka "Execute"). This instruction is invoked on every token transfer, and is where we implement our custom logic.

When the Token Extension program CPIs our program, it will invoke this instruction and pass to it all the accounts plus the amount of the transfer. The first 5 accounts will always be source, mint, destination, owner, and extraAccountMetaList, and the rest are the extra accounts that we added to the `ExtraAccountMetaList` account if there is any. You can see this ordering in the following example struct `TransferHook`:
...
As mentioned in the comment, the order here matters. We should pass the first 5 accounts as shown above, then the rest of the accounts should follow the order of the accounts in the `extraAccountMetaList` account.

As you know, Anchor will take it from here and make sure that everything is validated and parsed correctly, and it will pass the accounts to the instruction.

Now let's take a look at the transfer hook function itself. Here is a the function's signature:
...
We can add any logic we want here. We could fail the transfer if a specific condition is not met, or we could make another CPI from here to another program. The possilbilities are endless.

It's important to remember that the transfer hook gets called __after__ the transfer happens. When the transfer hook is getting invoked, the tokens have already left the sender account and get to the receiver account.
