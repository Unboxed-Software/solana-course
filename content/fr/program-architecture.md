...
Ici, dans la fonction `run_concept_shared_account`, au lieu de transférer vers le goulot d'étranglement, nous transférons vers le PDA `donation_tally`. De cette manière, nous n'affectons que le compte du donateur et son PDA - donc pas de goulot d'étranglement ! De plus, nous conservons une totalisation interne du nombre de lamports qui doivent être récupérés, c'est-à-dire transférés du PDA au portefeuille communautaire ultérieurement. À un moment donné dans le futur, le portefeuille communautaire passera et récupérera tous les lamports traînants (probablement une bonne tâche pour [clockwork](https://www.clockwork.xyz/)). Il est important de noter que n'importe qui devrait pouvoir signer pour la fonction de récupération, car le PDA a la permission sur lui-même.
